...
$q_order = UOJRequest::get('order', function ($x) {
        return in_array($x, ['latest', 'best']);
}, 'latest');
...
$pag = new Paginator([
        ...
        'cond' => $conds,
        'tail' => $q_order === 'best'
                ? 'order by score desc, submit_time asc, id asc'
                : 'order by id desc',
]);
?>
...
<div class="d-none d-sm-block mb-3">
        <form id="form-search" class="row gy-2 gx-3 align-items-end mb-3" target="_self" method="GET">
                ...
                <div class="col-auto">
                        <label for="input-order" class="form-label">排序方式</label>
                        <select class="form-select form-select-sm" id="input-order" name="order">
                                <option value="latest" <?= $q_order === 'latest' ? 'selected' : '' ?>>最新提交</option>
                                <option value="best" <?= $q_order === 'best' ? 'selected' : '' ?>>最优解</option>
                        </select>
                </div>
                <div class="col-auto">
